...
- 주문을 생성한다.
    ...
    - [ ] 매장식사(EAT_IN)의 경우 주문 테이블이 점유되어야 한다.


- 주문을 수락한다.
    - [ ] 주문 수락 전 상태는 대기(WAITING)여야 한다.
    - [ ] 주문 타입이 배달이면 주문Id, 주문 항목 가격 합계, 배달 주소를 이용해 배달을 요청한다.
...
- 주문을 완료한다.
    ...
    - [ ] 주문 타입이 매장식사(EAT_IN)일 경우, 주문 테이블은 비점유로 변경된다.
...
- 주문 테이블을 생성한다.
    - [ ] 주문 테이블 이름은 필수 입력값이다.
    - [ ] 테이블 손님 수는 0명, 비점유 상태로 생성된다.
- 주문 테이블을 앉음 처리한다.
    - [ ] 점유됨 처리한다.
- 주문 테이블을 정리한다.
    - [ ] 테이블 손님 수는 0명, 비점유 상태로 처리한다.
